...
function buildBody({ protocol, fieldMap, messages, systemPrompt, sessionId, config }) {
...
async function attempt(config, candidate, { messages, systemPrompt, sessionId }) {
 ...
 * >}
 */
export async function askLocalBrain({ messages, systemPrompt, sessionId } = {}) {
  ...
  for (const candidate of candidates) {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      const result = await attempt(config, current, { messages, systemPrompt, sessionId });
